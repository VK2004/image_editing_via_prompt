...
try:
    with open("promptdata.pickle", "rb")as f:
        words, labels, training, output = pickle.load(f)

except:

    words = [] # This is list of each word
    labels = []
    docs_x = [] # This is list of each word
    docs_y = []

    for intent in data["intents"]:
        for pattern in intent["prompts"]:
            wrds = nltk.word_tokenize(pattern) # This is list of each word
            words.extend(wrds)
            docs_x.append(wrds) 
    ...
    with open("promptdata.pickle", "wb")as f:
